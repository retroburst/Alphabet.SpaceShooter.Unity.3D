...
public class Done_GameController : MonoBehaviour
{
	...
	void Start ()
	{
		...
		currentPlayer.GetComponent<Done_PlayerController> ().ShotFired += ShotFiredEventHandler;
	...
	{ 
		...
		{
			Done_MoverTextEnemy mover = newHazard.transform.FindChild("Letter").GetComponent<Done_MoverTextEnemy>();
		...
		{
			Done_Mover mover = newHazard.GetComponent<Done_Mover>();
			float speed = Math.Abs(mover.speed);
			float speedIncrement = ((ENEMY_MAX_SPEED - speed) / alphabet.Length);
			mover.speed -= (speedIncrement * alphabetIndex);
			mover = null;
	...
	public void RegisterDestroyByContactForObservation (Done_DestroyByContact dbc)
	{
